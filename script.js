...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const waitlistId = 'cm8spk2h20002iy2j9bdmh129';
    ...
    if (mainWaitlistForm) {
        mainWaitlistForm.addEventListener('submit', async function(e) {
            ...
            // Prepare metadata with pet information
            const metadata = {};
            if (petName) metadata.petName = petName;
            if (petType) metadata.petType = petType;
            ...
            try {
                ...
                const response = await fetch(`https://www.waitlist.email/api/waitlists/${waitlistId}/subscribers/create`, {
                    method: 'POST',
                    headers: {
                ...
                // Success
                mainFormSuccess.textContent = 'You have been added to the waitlist! Check your email for confirmation.';
                mainWaitlistForm.reset();
                
            } catch (error) {
                mainFormError.textContent = error.message || 'Something went wrong. Please try again.';
            } finally {
    ...
    if (waitlistForm) {
        waitlistForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Success
                formSuccess.textContent = 'You have been added to the waitlist! Check your email for confirmation.';
                waitlistForm.reset();
    ...
    function smoothScrollTo(targetId) {
        ...
        if (targetElement) {
            ...
            const targetPosition = targetElement.getBoundingClientRect().top + window.pageYOffset - headerBottomWithPadding;
    ...
    const otherWaitlistForms = document.querySelectorAll('.waitlist-form:not(.hero-cta .waitlist-form)');
    
    otherWaitlistForms.forEach(form => {
        if (form !== mainWaitlistForm) { // Avoid handling the same form twice
    ...
    if (testimonials.length > 1) {
        ...
        testimonials.forEach((_, index) => {
            ...
            dot.style.backgroundColor = index === 0 ? 'var(--primary-color)' : 'var(--border-color)';
    ...
    function showTestimonial(index) {
        ...
        dots.forEach((dot, i) => {
            dot.style.backgroundColor = i === index ? 'var(--primary-color)' : 'var(--border-color)';
        });
    ...
    const revealElements = document.querySelectorAll('.feature-card, .step, .plan-card, .point');
